...
const LinkButton = ({ type, path, text, icon, record, onClick }) => {
  ...
  if (type === 'link')
    return (
      <Link key={linkUrl} to={linkUrl} onClick={(e) => onClick(record, e)}><Icon type={icon} /> {text}</Link>
    )
  else
    return (
      <Link to={linkUrl}>
...
export const Column = (title, dataIndex, sorter = false, options) => {
    return Object.assign({title, dataIndex, key: dataIndex, sorter, visible: true}, options)
}
    ...
    <div>
      {
        actions.map( (act, idx) => {
          ...
          return <span key={idx}>{ActionButton(props)}</span>
        })
      }
    </div>
  )

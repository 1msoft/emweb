...
import { Table, Pagination, Button, Popconfirm, Input, Checkbox, Switch } from 'antd'
...
class PaginationTable extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!_.isEqual(this.props.currentPage, nextProps.currentPage)) {
      this.setState({ jumper: `${nextProps.currentPage}` });
    }
  }
    ...
    this.columns = this.columns.map((value, idx) => {
      const data = {
        ...
        render(text, record, idx) {
          const cache = {}
          cache.text = typeof value.formatter === 'function' ?
            value.formatter(text, record, idx) : text
          return (
            <div>
              {
                self.state.editable[idx] ?
                  <Input type="text"
                    defaultValue={cache.text}
                    onChange={(e) => self.handleEdit(idx, value.key, e.target.value)} />
                  :
                  <span>{cache.text}</span>
              }
            </div>
          )
        }
  ...
  render() {
    ...
    const pagination = Object.assign({
      ...
      defaultCurrent: 1,
      defaultPageSize: this.props.defaultPageSize || this.state.pageSize,
    ...
    const render_columns = this.props.isSelectColumns ?
      ...
      : this.columns
    const props = Object.assign({}, this.props, this.config, {
        ...
        <Table {...props} />
        {
          pageType ?
            <div className='clearfix'>
              <ul
                style={{ clear: 'none', padding: '12px 0', float: 'right' }}
